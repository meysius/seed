...
import { logErrorForDebugging } from './errorHandler';
...
const baseUrls = {
  ...
  IDENTITY: getConfig().serverRuntimeConfig.BACKEND_BASE_URL,
};

export class ApiError extends Error {
  type: 'NETWORK' | 'APPLICATION' | 'UNKNOWN' | 'RESPONSE_PARSING_ERROR';
  ...
  public userFriendlyMessage(): string {
    if (this.type === 'APPLICATION' && this.applicationError) {
      ...
      if (this.applicationError.id === 'InternalServerError') {
...
export async function callApi<ResponseType>(
  ...
  if (!response.ok) {
    ...
    try {
      ...
      throw new ApiError({ type: 'RESPONSE_PARSING_ERROR', causedBy: error });
    }
    throw new ApiError({
      type: 'APPLICATION',
      applicationError,
